...
bool VariableTexture::loadFromFile(const string &filename, PixelFormat imageFormat)
{
	...
	case TEXTURE_PIXEL_FORMAT_RGB:
		image = SOIL_load_image(filename.c_str(), &widthTex, &heightTex, &channels, SOIL_LOAD_RGB);
		break;
	case TEXTURE_PIXEL_FORMAT_RGBA:
		image = SOIL_load_image(filename.c_str(), &widthTex, &heightTex, &channels, SOIL_LOAD_RGBA);
		break;
	case TEXTURE_PIXEL_FORMAT_L:
		image = SOIL_load_image(filename.c_str(), &widthTex, &heightTex, &channels, SOIL_LOAD_L);
		break;
	}
	if(image == NULL)
		return false;
